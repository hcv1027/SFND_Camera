...
void magnitudeSobelSol() {
  ...
  cv::filter2D(blurred, result_x, -1, kernel_x, cv::Point(-1, -1), 0,
               cv::BORDER_DEFAULT);
  cv::filter2D(blurred, result_y, -1, kernel_y, cv::Point(-1, -1), 0,
               cv::BORDER_DEFAULT);
  ...
  // show result
  cv::Mat img_array[] = {imgGray, magnitude};
  cv::Mat concate_img;
...
void magnitudeSobel() {
  ...
  // cv::GaussianBlur(imgGray, blurred, cv::Size(5, 5), sigma, sigma,
  //                  cv::BORDER_DEFAULT);
  cv::GaussianBlur(imgGray, blurred, cv::Size(5, 5), sigma);
  ...
  cv::filter2D(blurred, result_x, -1, kernel_x, cv::Point(-1, -1), 0,
               cv::BORDER_DEFAULT);
  cv::filter2D(blurred, result_y, -1, kernel_x, cv::Point(-1, -1), 0,
               cv::BORDER_DEFAULT);
